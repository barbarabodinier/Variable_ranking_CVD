QRiskFemale=function(age,b_AF,b_atypicalantipsy,b_corticosteroids,b_migraine,
                     b_ra,b_renal,b_semi,b_sle,b_treatedhyp,b_type1,b_type2,
                     bmi,ethrisk,fh_cvd,rati,sbp,sbps5,
                     smoke_cat,town){
  Iethrisk=c(0, # white or not recorded
  ...
  # NB: Number checked by comparing exp(beta) to the hazard ratio published in paper (p.14)
  
  Ismoke=c(0, # non smoker
           0.1338683378654626200000000, # former smoker 
           0.5620085801243853700000000, # light smoker 
           0.6674959337750254700000000, # moderate smoker 
  ...
  dage = age
  dage=dage/10
  age_1 = dage^(-2) #pow(dage,-2)
  age_2 = dage
  dbmi = bmi
  dbmi=dbmi/10
  bmi_1 = dbmi^(-2) #pow(dbmi,-2)
  bmi_2 = dbmi^(-2)*log(dbmi) #pow(dbmi,-2)*log(dbmi)
  ...
  score = 100.0 * (1 - 0.988876402378082^exp(a))
  return(c(a,score))
}


QRiskMale=function(age, 
                   ...
                   b_type1=0, b_type2=0, 
                   bmi, ethrisk=1, fh_cvd=0, rati, 
                   sbp, sbps5, smoke_cat=1, town=0){
  ...
  dage = age
  dage=dage/10
  age_1 = dage^(-1) #pow(dage,-1)
  age_2 = dage^3 #pow(dage,3)
  dbmi = bmi
  dbmi=dbmi/10
  bmi_2 = dbmi^(-2)*log(dbmi) #pow(dbmi,-2)*log(dbmi)
  bmi_1 = dbmi^(-2) #pow(dbmi,-2)
  ...
  ## Calculate the score itself
  score = 100.0 * (1 - 0.977268040180206^exp(a)) #pow(survivor[surv], exp(a)) )
  return(c(a,score))
}
