...
GND.calib <- function(pred, tvar, out, cens.t, groups, adm.cens) {
  ...
  if (any(kmtab[, 5] == -1)) {
    stop("Stopped because at least one of the groups contains <2 events. Consider collapsing some groups.")
...
Consider collapsing some groups to avoid this problem.")
  ...
  c(
    df = numcat - 1, chi2gw = sum(hltab$GND_component), pvalgw = pchisq(sum(hltab$GND_component), numcat - 1, lower.tail = FALSE),
