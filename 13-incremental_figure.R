  ...
  for (j in 1:length(outcome_names)) {
    ...
    background <- TRUE
    {
      ...
      for (j in 1:length(genders)) {
        ...
        # Reading the output
        c_test_lasso <- readRDS(paste0("Results/C_statistic_incremental_test_", outcome, "_m", model_id, "_", data_input, "_", gender, ".rds"))
        if (model_id == 0) {
        ...
        c_test_pce <- readRDS(paste0("Results/C_statistic_incremental_test_", outcome, "_m", model_id, "_PCE_", data_input, "_", gender, ".rds"))
        i <- i + 1
